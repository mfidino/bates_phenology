...
# required packages
packs <- c("dplyr", "MCMCpack", "mcmcplots",
           "runjags", 'parallel')
...
co <- read.csv("./data/co2.csv", header = TRUE)
...
sample_steps = 10000
thin_steps = 2
mod_mcmc <- as.mcmc.list(run.jags( model= "./jags_models/climate_resid_model.R" , 
                                           monitor=params , 
                                           data=dl ,  
                                           n.chains=n_chains ,
                                           adapt=adapt_steps ,
...
pdf("climate_resid_plot.pdf", height = 5, width = 5)
